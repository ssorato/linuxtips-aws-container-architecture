...
resource "aws_appautoscaling_target" "idempotency_read" {
  ...
  tags = merge(
    {
      Name = format("%s-appautoscaling-read", lookup(var.dynamodb_idempotency, "name"))
    },
    var.common_tags
  )
}
...
resource "aws_appautoscaling_target" "idempotency_write" {
  ...
  tags = merge(
    {
      Name = format("%s-appautoscaling-write", lookup(var.dynamodb_idempotency, "name"))
    },
    var.common_tags
  )
}
...
resource "aws_dynamodb_table_replica" "idempotency" {
  ...
  tags = merge(
    {
      Name = format("%s-replica", lookup(var.dynamodb_idempotency, "name"))
    },
    var.common_tags
  )
}