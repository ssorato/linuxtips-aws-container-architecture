...
resource "aws_appautoscaling_target" "sales_read" {
  ...
  tags = merge(
    {
      Name = format("%s-appautoscaling-read", lookup(var.dynamodb_sales, "name"))
    },
    var.common_tags
  )
}
...
resource "aws_appautoscaling_target" "sales_write" {
  ...
  tags = merge(
    {
      Name = format("%s-appautoscaling-write", lookup(var.dynamodb_sales, "name"))
    },
    var.common_tags
  )
}
...
resource "aws_dynamodb_table_replica" "sales" {
  ...
  tags = merge(
    {
      Name = format("%s-replica", lookup(var.dynamodb_sales, "name"))
    },
    var.common_tags
  )
}
